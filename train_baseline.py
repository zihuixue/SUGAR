...
def gen_model(args):
    if args.use_labels:
        model = GCN(in_feats + n_classes, args.n_hidden, n_classes, args.n_layers, F.relu, args.dropout, args.use_linear)
    else:
...
def train(model, graph, labels, train_idx, optimizer, use_labels):
    ...
    if use_labels:
        ...
        train_pred_idx = train_idx[mask].long()
...
def run(args, graph, labels, train_idx, val_idx, test_idx, evaluator, n_running):
    ...
    for epoch in range(1, args.n_epochs + 1):
        ...
        if epoch % args.log_every == 0:
            gpu_mem_alloc = th.cuda.max_memory_allocated() / (1024 ** 3)
            print(f'GPU memory allocated: {gpu_mem_alloc:.2f} GBs')
            print(
                f"Run: {n_running}/{args.n_runs}, Epoch: {epoch}/{args.n_epochs}, Average epoch time: {total_time / (epoch-8) * 100:.2f}ms\n"
...
def main():
    ...
    print(f'node | edge num: {graph.number_of_nodes()} | {graph.number_of_edges()}')
